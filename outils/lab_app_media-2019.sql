...
UPDATE utilisateur set mot_de_passe = SHA2(mot_de_passe, 224);

SELECT * FROM utilisateur;

SELECT f.pk_facture, c.prenom, c.nom, f.date_service, f.paiement_status, f.no_confirmation 
FROM facture AS f
INNER JOIN client c ON f.fk_client = c.pk_client
ORDER BY f.date_service DESC;
...
SELECT tarif_facture FROM ta_facture_service
WHERE fk_facture = 16;

SELECT * FROM service
WHERE service_titre LIKE '%bases%' OR service_description LIKE '%bases%';
